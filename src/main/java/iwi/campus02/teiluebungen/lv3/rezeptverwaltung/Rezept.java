...
public class Rezept {
    ...
    public void printRezept(){
        ...
        for(int i = 0; i < this.zutaten.length; i++){
            System.out.print(zutaten[i].toString());
        }
    }

    public Rezept umrechnen(int personen){
        double factor = personen / (getPersonen() * 1.0);
        Zutat[] neueZutaten = new Zutat[this.zutaten.length];

        for (int i = 0; i < this.zutaten.length; i++){
            neueZutaten[i] = new Zutat((this.zutaten[i].getName()),
                    (int) (this.zutaten[i].getMenge() * factor));
        }

        Rezept result = new Rezept(this.name, personen, neueZutaten);
        return result;
    }
